...
const ChampionSelect = () => {
  ...
  const champion: Champion =
    champions.find((champion) => champion.key === championId) ?? null;

  return (
    <div className="flex flex-col p-16 space-y-8">
      <div>
        <button className="text-3xl text-white" onClick={() => router.back()}>
          <IoArrowBackOutline />
        </button>
      </div>
        ...
        <>
          <div className="flex space-x-4 px-8">
            ...
            <div className="flex flex-col justify-center">
              ...
              <span className="text-white text-xs">{champion.title}</span>
            </div>
          </div>
          ...
          <div className="flex px-8">
            <div className="flex flex-col space-y-8">
              <ChampionSkillsTable />
              <ItemsList
                itemsNumberList={[6630, 3047, 3161, 3053, 6333, 3026]}
              />
            </div>
